...
Current backend rules are implemented in `RATING_RULES` and `computeReviewType`.

- 🔴 **User Input: FORGOT**
  - **Resulting State:** `FULL_RECODE` (resolved by `computeReviewType(stabilityScore)`)
  - **Logic:** The user completely forgot the approach.
  - **Math:** **Hard reset** in `RATING_RULES.FORGOT`: `stabilityScore = 10`.
  - **Interval:** Resets to a **1-day** interval to force immediate re-learning.
- 🟡 **User Input: SLOW**
  - **Resulting State:** `PATTERN_REBUILD` (resolved by `computeReviewType(stabilityScore)`)
...
- 🟢 **User Input: CLEAN**
  - **Resulting State:** `MICRO_RECALL` (resolved by `computeReviewType(stabilityScore)`)
...
- `({ userId: 1, url: 1 }, { unique: true, partialFilterExpression: { isDeleted: false } })`
  ...
  - Matches implementation in `backend/src/modules/problems/problem.model.js`.
...
sequenceDiagram
  ...
  API->>API: Algorithm: RATING_RULES.CLEAN => stability +20 (cap 100), interval x2.5 then Math.round
  API->>DB: Update (example: stability 30 -> 50, nextReview: +3 days, rounded from 2.5)
